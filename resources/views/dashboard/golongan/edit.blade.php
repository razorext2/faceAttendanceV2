...
<div class="w-full">
    <form action="{{ route('golongan.update', $golongan) }}" class="mt-4" method="POST">
        @csrf
        @method('put')
                    ...
                    <div class="max-w-xl">
                        ...
                        <div class="grid md:grid-cols-2 gap-4 sm:gap-6 my-4">
                            <div class="w-full">
                                ...
                                <input type="text" name="nama_golongan" id="nama_golongan" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Nama Golongan" required="" value="{{ old('nama_divisi', $golongan->nama_golongan ?? '') }}">
                            </div>
                            <div class="w-full">
                                <label for="alias" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alias</label>
                                <input type="text" name="alias" id="alias" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Alias" required="" value="{{ old('alias', $golongan->alias ?? '') }}">
                            </div>
                        </div>
                            ...
                            <div class="w-full border-b-gray-500 border-b">
                                <div class="grid sm:flex gap-2 md:gap-4 py-4">
                                    <div class="sm:flex-none w-20">
                                        <h3 class="text-md font-semibold text-gray-700 dark:text-white mt-0 md:mt-9">{{ ucfirst($day) }}</h3>
                                    </div>
                                    ...
                                    <div class="sm:flex-1 w-full">
                                        ...
                                        <input type="time" name="jam_masuk_{{ strtolower($day) }}" id="jam_masuk_{{ strtolower($day) }}"
                                            class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 block w-full p-2.5"
                                            required
                                            value="{{ $jadwal[$day]->jam_masuk }}">
                                    </div>

                                    <div class="sm:flex-1 w-full">
                                        <label for="jam_keluar_{{ strtolower($day) }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jam Keluar</label>
                                        <input type="time" name="jam_keluar_{{ strtolower($day) }}" id="jam_keluar_{{ strtolower($day) }}"
                                            class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 block w-full p-2.5"
                                            required
                                            value="{{ $jadwal[$day]->jam_keluar }}">
                                    </div>
