	...
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			...
			function getTimeline() {
				...
				fetch(
						`/api/get-attendance-data?kode_pegawai={{ Request::segment(4) }}&d={{ \Carbon\Carbon::parse(Request::query('d'))->isoFormat('Y-MM-DD') }}`
					)
		...
		document.getElementById('dateForm').addEventListener('submit', function(event) {
			...
			if (dateInput) {
				url.searchParams.set('d', dateInput); // Add or update the 'd' parameter with the date value
			}
