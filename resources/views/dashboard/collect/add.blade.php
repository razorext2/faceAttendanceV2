		...
		<div
			...
			<div class="w-full">
				...
				<form class="mt-4" method="POST">
					...
					<input id="kode_pegawai" name="kode_pegawai" type="hidden" value="28101999">
					...
					<div class="relative mb-4 w-full">
						...
						<input class="block w-full rounded-lg border border-gray-300 bg-white p-2.5 text-sm text-gray-900" id="title"
							id="title" name="title" type="text" placeholder="Judul laporan.." required="">
					</div>

					<!-- Tombol Ambil Gambar -->
					<div class="relative mb-4 w-full">
						<label class="dark:text-white mb-2 block text-sm font-medium text-gray-900"
							for="capture-button">Dokumentasi</label>
						<button
					...
					<div class="w-full overflow-x-auto">
						<div class="flex space-x-2" id="captured-images">
	...
	<script>
			...
			$('#store').click(function(e) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						$('#title').val('');
						$('#keterangan').val('');
						document.querySelector('.ql-editor').classList.add('hidden');
						...
						// redirect
						setTimeout(() => {
							window.location.href = "{{ route('collect.index') }}"
						}, 1200);
					},
					error: function(xhr, status, error) {
						...
						Swal.fire({
							...
							text: xhr.responseJSON.message || "Ada kesalahan pada permintaan data."
						});
					}
				});
			});
		...
		function backCameraStream() {
			...
			captureImageButton.addEventListener("click", function() {
				...
				// Menambah margin kontainer keterangan
