...
function captureImage() {
    ...
    return (
        e.getContext("2d").drawImage(video, 0, 0, e.width, e.height),
        new Promise((t) => {
            e.toBlob((e) => {
                t(e || null);
            }, "image/png"),
...
function displayImageOnCanvas(e) {
    ...
    if (t) {
        ...
        (n.src = e),
            (n.onload = () => {
                (canvInfo.width = n.width),
                    (canvInfo.height = n.height),
                    t.drawImage(n, 0, 0);
            }),
            (n.onerror = () => {
...
function displayPegawaiData(e) {
    ...
    const t = pegawaiInfo,
        n = e.jabatan_relasi
            ? e.jabatan_relasi.nama_jabatan
            : "Jabatan tidak ditemukan",
        a = e.golongan_relasi
            ? e.golongan_relasi.nama_golongan
...
function jamMasuk() {
    const e = new Date().toLocaleString("id-ID", {
        year: "numeric",
        month: "long",
        day: "numeric",
        hour: "numeric",
    ...
    document.getElementById("waktu-masuk").textContent = `Waktu Masuk: ${e}`;
}
...
function jamKeluar() {
    ...
    document.getElementById("waktu-keluar").textContent = `Waktu Keluar: ${e}`;
}

function formatDatabaseDate(e) {
    ...
    if (!detectionInterval)
        try {
            ...
            if (
                ...
                return void console.error("No labeled face descriptors found.");
            const t = new faceapi.FaceMatcher(e),
                n = { width: video.width, height: video.height };
            faceapi.matchDimensions(overlay, n);
            const a = overlay.getContext("2d", { willReadFrequently: !0 });
                ...
                try {
                    ...
                    for (const e of o) {
                        ...
                        if (
                            ...
                            new faceapi.draw.DrawBox(t, {
                                ...
                                (i = !0),
                                (r = n.label),
                                detectedFaces.add(n.label);
                            const a = await getPegawaiDataByLabel(r);
                            a &&
                                (displayPegawaiData(a),
                                (c = a.kode_pegawai),
                                (l = a.nik_pegawai));
                        }
                    }
                    if (i && c) {
                        ...
                        if (e) {
                            ...
                            await saveImageToServer(e, r),
                                await saveAttendance(c, l, t),
                                (canvInfo.style.display = "block");
                        } else console.error("Failed to capture image");
                    }
                    a.clearRect(0, 0, overlay.width, overlay.height),
                        a.drawImage(s, 0, 0);
                } catch (e) {
                    console.error(
...
async function saveImageToServer(e, t) {
    ...
    try {
        ...
        if (e.ok) {
            ...
            t.imageUrl &&
                (displayImageOnCanvas(t.imageUrl),
...
async function saveAttendance(e, t, n) {
    try {
        const n = await fetch("/check-attendance", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": csrfToken,
                },
                body: JSON.stringify({ kode_pegawai: e, nik_pegawai: t }),
            }),
            a = await n.json();
        if (a.hasClockedIn) {
            const n = formatDatabaseDate(a.jam_masuk);
            (document.getElementById(
                "waktu-masuk"
            ).textContent = `Waktu Masuk: ${n}`),
                jamKeluar();
            const o = await fetch("/store-attendance-out", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": csrfToken,
                ...
                s = await o.json();
            s.success
                ? (console.log(s.message), attendanceAlert())
                : console.error("Failed to record clock-out:", s.message);
        } else {
            const n = new Date(),
                a = {
                    year: "numeric",
                ...
                o = n.toLocaleString("id-ID", a);
            (document.getElementById(
                "waktu-masuk"
            ).textContent = `Waktu Masuk: ${o}`),
                (document.getElementById("waktu-keluar").textContent =
                    "Jam Keluar: Belum ada data");
            const s = await fetch("/store-attendance", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": csrfToken,
                    ...
                    body: JSON.stringify({
                        kode_pegawai: e,
                        nik_pegawai: t,
                        jam_masuk: n.toISOString(),
                    }),
                }),
                i = await s.json();
            i.success
                ? (console.log(i.message), attendanceAlert())
                : console.error("Failed to record clock-in:", i.message);
        }
    } catch (e) {
        console.error("Error checking or saving attendance:", e);
    }
}
