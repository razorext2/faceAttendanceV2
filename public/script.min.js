...
async function saveAttendance(e, t, n) {
  try {
    ...
    if (a.hasClockedIn) {
      const n = a.jam_masuk;
      document.getElementById("waktu-masuk").textContent = `${n}`;
      const o = await fetch("/store-attendance-out", {
        ...
        s = await o.json();
      s.success ? (console.log(s.message), attendanceAlert()) : console.error("Failed to record clock-out:", s
        .message)
    } else {
      const n = new Date,
        a = {
      ...
      document.getElementById("waktu-masuk").textContent = `${o}`;
      const s = await fetch("/store-attendance", {
        ...
        i = await s.json();
      i.success ? (console.log(i.message), attendanceAlert()) : console.error("Failed to record clock-in:", i.message)
    }
  } catch (e) {
